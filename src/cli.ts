...
const argv = await yargs(hideBin(process.argv))
	...
	.option("outputIdsFolder", {
		type: "string",
		description: "Output folder for IDs file (defaults to outputSpriteFolder)",
	})
	...
	.option("outputConstsFolder", {
		type: "string",
		description:
			"Output folder for constants file (defaults to outputSpriteFolder)",
	})
...
async function main() {
	try {
		const config: SvgIconSpriteConfig = {
			...
			// Use outputSpriteFolder as default for other folders if not provided
			outputIdsFolder: argv.outputIdsFolder || argv.outputSpriteFolder,
